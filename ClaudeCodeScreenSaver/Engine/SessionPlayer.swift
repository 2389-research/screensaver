// ABOUTME: State machine that replays parsed SessionEvent arrays into terminal line output.
...
final class SessionPlayer {
    ...
    private func tick(_ dt: TimeInterval) -> TimeInterval {
        ...
        case .typingPrompt(let charIndex, let elapsed):
            return tickTyping(
                ...
                charsPerSec: Self.promptCharsPerSec, prefix: "> ",
        ...
        case .typingResponse(let charIndex, let elapsed):
            return tickTyping(
                dt: dt, charIndex: charIndex, elapsed: elapsed,
                charsPerSec: Self.responseCharsPerSec, prefix: "",
        ...
        case .typingToolCall(let charIndex, let elapsed):
            return tickTyping(
                dt: dt, charIndex: charIndex, elapsed: elapsed,
                charsPerSec: Self.toolCallCharsPerSec, prefix: "",
    ...
    private func tickTyping(
        ...
        updateLinesForTyping(text: typedText, prefix: prefix)

        if newCharIndex >= totalChars {
            // Typing complete, finalize with full text
            updateLinesForTyping(text: currentEventText, prefix: prefix)
    ...
    private func updateLinesForTyping(text: String, prefix: String) {
        ...
        let endIndex = currentEventLineStart + prefixedLines.count
        if allLines.count < endIndex {
            ...
            for (i, line) in prefixedLines.enumerated() {
